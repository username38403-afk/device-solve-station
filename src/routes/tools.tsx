...
import { FileImage, FileUp, ImageDown, Zap, Download, RotateCcw, AlertCircle } from "lucide-react";
...
function ToolsPage() {
  ...
  const reset = () => { setFile(null); setOutputUrl(null); setOutputSize(null); setError(null); if (inputRef.current) inputRef.current.value = ""; };
    ...
    <ToolWorkspace eyebrow="Utility bench" title="Useful tools. Private by default." description="Handle common file tasks in the browser, with a clear view of what is happening.">
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <WorkspacePanel title="Tool workspace" description={tool ? "Choose a local file, process it in your browser, and download the result." : "Choose a tool to begin."}>
         {tool ? <div className="mt-5 space-y-4"><input ref={inputRef} type="file" accept="image/png,image/jpeg,image/webp" className="block w-full rounded-2xl border border-line bg-mist p-3 text-sm" onChange={(event) => { setFile(event.target.files?.[0] ?? null); setOutputUrl(null); setError(null); }} /><div className="flex flex-wrap items-center gap-3"><label htmlFor="quality" className="text-xs font-semibold text-soft">Quality {Math.round(Number(quality) * 100)}%</label><input id="quality" type="range" min="0.35" max="1" step="0.01" value={quality} onChange={(event) => setQuality(event.target.value)} className="w-40 accent-[var(--royal)]" /><Button type="button" variant="brand" onClick={processFile} disabled={busy || !file}><ImageDown /> {busy ? "Processing…" : "Process locally"}</Button><Button type="button" variant="ghost" onClick={reset}><RotateCcw /> Reset</Button></div>{error && <p className="flex items-center gap-2 rounded-2xl bg-rose-soft p-3 text-sm text-rose"><AlertCircle className="size-4" />{error}</p>}{outputUrl && <div className="rounded-2xl bg-mint-soft p-4 text-sm text-mint"><p className="font-semibold">Ready to download</p><p className="mt-1 text-xs">Original: {(file?.size ?? 0 / 1024).toFixed(1)} KB · Output: {((outputSize ?? 0) / 1024).toFixed(1)} KB</p><a href={outputUrl} download={`fixmytech-${file?.name ?? "image"}.jpg"} className="mt-3 inline-flex min-h-10 items-center gap-2 rounded-md bg-mint px-4 py-2 text-sm font-semibold text-ink"><Download className="size-4" /> Download result</a></div>}</div> : <div className="mt-5 grid grid-cols-2 gap-3"><div className="rounded-2xl bg-lavender p-4 text-indigo"><ImageDown className="size-5" /><p className="mt-3 text-xs font-semibold">Local processing</p></div><div className="rounded-2xl bg-sun-soft p-4 text-sun"><Zap className="size-5" /><p className="mt-3 text-xs font-semibold">Clear output</p></div></div>}
      </WorkspacePanel>
    </ToolWorkspace>
  );
}