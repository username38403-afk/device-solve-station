  ...
  return (
    <ToolWorkspace eyebrow="Command search" title="Find the right tool." description="Search diagnostics, utilities, safety checks, and device tests.">
      <WorkspacePanel title="Search workspace" description="Type a symptom or task, then open the closest match.">
        ...
        {results.length ? <div className="mt-5 grid gap-3">{results.map((item) => { const Icon = item.icon; const openLink = "search" in item ? <Link to={item.to} search={item.search}>Open tool</Link> : <Link to={item.to}>Open tool</Link>; return <div key={item.title} className="flex flex-col gap-3 rounded-2xl border border-line bg-surface p-4 sm:flex-row sm:items-center"><span className="grid size-10 shrink-0 place-items-center rounded-2xl bg-lavender text-indigo"><Icon className="size-5" /></span><span className="min-w-0 flex-1"><strong className="block font-display text-lg">{item.title}</strong><span className="mt-1 block text-xs text-soft">{item.description}</span></span><Button asChild variant="quiet" className="shrink-0">{openLink}</Button></div>; })}</div> : <div className="mt-5 rounded-2xl bg-sun-soft p-4 text-sm text-sun">No matching tools. Try a broader symptom such as “wifi”, “file”, or “camera”.</div>}
      </WorkspacePanel>
    </ToolWorkspace>
  );
}